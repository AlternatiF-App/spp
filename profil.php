...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['sub'] )){
		...
		if(isset($_REQUEST['submit'])){
			...
			$sql = mysqli_query($con, "SELECT password FROM user WHERE iduser='$iduser' AND password=md5('$pass1')");
			if(mysqli_num_rows($sql) > 0){
				$do = mysqli_query($con, "UPDATE user SET password=md5('$pass2') WHERE iduser='$iduser'");
				if($sql > 0){
