	...
	*/
	...
	if(isset($_REQUEST['submit'])){
		...
		if($submit=='bayar'){
			...
			$qbayar = mysqli_query($con, "INSERT INTO pembayaran VALUES('$kls','$nis','$bln','$tgl','$jml')");
		...
		if($submit=='hapus'){
			...
			$qbayar = mysqli_query($con, "DELETE FROM pembayaran WHERE kelas='$kls' AND nis='$nis' AND bulan='$bln'");
		...
		$qsiswa = mysqli_query($con, "SELECT * FROM siswa WHERE nis='$nis'");
		list($nis,$nama,$idrombel) = mysqli_fetch_array($qsiswa);
...
<form class="form-inline" role="form" method="post" action="./admin.php?hlm=bayar">
  ...
  <div class="form-group">
	  ...
	  <?php
		$qkelas = mysqli_query($con, "SELECT kelas,th_pelajaran FROM kelas WHERE nis='$nis'");
...
<?php
		...
		$qbayar = mysqli_query($con, "SELECT kelas,bulan,tgl_bayar,jumlah FROM pembayaran WHERE nis='$nis' ORDER BY tgl_bayar DESC");
