...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		if(($submit=='simpan') AND isset($_REQUEST['nis'])){
			$nis = $_REQUEST['nis'];
			$sql = mysqli_query($con, "INSERT INTO kelas VALUES('$kelas','$tapel','$nis')");
		}
		
		//proses hapus siswa dari kelas
		if(($submit=='hapus') AND isset($_REQUEST['nis'])){
			$nis = $_REQUEST['nis'];
			$qsiswa = mysqli_query($con, "DELETE FROM kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel' AND nis='$nis'");
		}
		...
		$qsiswa = mysqli_query($con, "SELECT nis,nama FROM siswa WHERE idrombel='$idrombel' AND nis NOT IN (SELECT nis FROM kelas ) ORDER BY nis");
		...
		$qkelas = mysqli_query($con, "SELECT nis FROM kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel'");
		if(mysqli_num_rows($qkelas) > 0){
			$no=1;
			while(list($nis)=mysqli_fetch_array($qkelas)){
				echo '<tr><td>'.$no.'</td>';
				echo '<td>'.$nis.'</td>';
				$qsiswa = mysqli_query($con, "SELECT nama FROM siswa WHERE nis='$nis'");
				list($siswa) = mysqli_fetch_array($qsiswa);
				echo '<td>'.$siswa.'</td>';
				//button hapus siswa
				echo '<td><a href="admin.php?hlm=master&sub=kelas&aksi=view&nis='.$nis.'&kelas='.$kelas.'&tapel='.$tapel.'&idrombel='.$idrombel.'&submit=hapus" class="btn btn-danger btn-xs">Hapus</a></td></tr>';
				$no++;
			}
		} else {
...
<form method="post" action="admin.php?hlm=master&sub=kelas&aksi=view" class="form-horizontal" role="form">
	...
	<div class="form-group">
		...
		<div class="col-sm-2">
			...
			<?php
				$qtapel = mysqli_query($con, "SELECT tapel FROM tapel ORDER BY tapel DESC");
