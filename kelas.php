...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['aksi'] )){
		...
		$sql = mysqli_query($con, "SELECT kelas.kelas, kelas.th_pelajaran, count(kelas.nis) as jml, siswa.idrombel FROM kelas,siswa WHERE kelas.nis=siswa.nis AND th_pelajaran='2018/2019' GROUP BY kelas.kelas");
		echo '<h2>Daftar Kelas</h2><hr>';
      echo '<div class="row">';
		echo '<div class="col-md-7"><table class="table table-bordered">';
		echo '<tr class="info"><th width="50">#</th><th>Kelas</th><th>Rombel</th>';
		...
		if( mysqli_num_rows($sql) > 0 ){
			...
			while(list($kelas,$tapel,$jumlah,$idrombel) = mysqli_fetch_array($sql)){
				...
				echo '<td>'.$rombel.' <span class="badge pull-right">'.$jumlah.' siswa</span></td>';
				echo '<td><a href="./admin.php?hlm=master&sub=kelas&aksi=view&kelas='.urlencode($kelas).'&tapel='.$tapel.'&idrombel='.$idrombel.'&submit=y" class="btn btn-success btn-xs">Edit</a> ';
